...
describe("SlackMessages", () => {
	describe("Message rendering", () => {
		const issue = {
			...
			body:
				"This is a very important issue with <unsafe> characters & stuff",
			repo: {
				owner: "atomisthq",
				name: "bot-service",
			},
		...
		describe("Given message with attachments and actions", () => {
			...
			const msg: SlackMessage = {
				text: `${url(
					ghUser.url,
					"@" + ghUser.name,
				)} opened issue: ${url(issue.url, issue.title)}`,
				attachments: [
					{
...
`;
	...
	describe("Urls", () => {
		it("can render links label", () => {
			assert.strictEqual(
				url("http://someplace", "some place"),
				"<http://someplace|some place>",
			);
		});

		it("can render links with undefined label", () => {
			assert.strictEqual(
				url("http://someplace", undefined),
				"<http://someplace>",
			);
		});
